...
class ruta:
    def __init__(self, nombre, modulo):
        ...
        self.modulo=modulo
        self.cliente=[]
    def agregarCompras(self, clinete):
        if len(self.compras)<20:
            self.compra.append(compras)
            return True
        else:
            return False
class farmacia:
    ...
    def estadoclientes(self, nombre, direccion):
        print("estado de las compras: ")
        for compra in compras:
            print(f"Nombre: {compra.nombre}{compra.direccion}-estado:{compra.estado}")
    ...
    def eliminarcliente(self, listacompra):
        idcompra=input("por favor ingrese el id del compra que desea eliminar: ")
        for compra in listacompra:
            if compra.id==idcompra:
                listacompra.remove(compra)
                print("Compra eliminada exitosamente")
                return
            print("No se encontro ninguna compra con ese id")
...
class reporte:
    def __init__(self):
        ...
        self.productos=[]
        self.ruta=[]
        self.pancamp=farmacia("2", "yessica","el puerto")
    def clientenuevo(self, compra):
        self.compras.append(compra)
    def rutanueva(self, ruta):
        rutanueva=ruta(id, nombre)
        self.ruta.append(rutanueva)
    def clienteruta(self, compra, rutanombre):
        for rutas in self.ruta:
            if ruta.agregarCompras(compra):
                return True
            else:
                return False
    def empleadonuevo(self, empleado):
        self.compras.append(empleado)
    def rutanueva(self, ruta):
        rutanueva=ruta(id, nombre, )
        self.ruta.append(rutanueva)
...
if opc=="1":
    ...
    if x==1:
        ...
        if x==1:
            id=int(input("ingrese el id de la nueva compra: "))
            nombre=int(input("ingrese el nombre de la nueva compra: "))
            direccion=int(input("ingrese la direccion de la nueva compra: "))
            nueva_compra={
                "id":id,
                "nombre":nombre,
                "direccion":direccion,
            ...
            miinfo=abrirarchivo()
            miinfo[0]["clientes"].append(nueva_compra)
            guardarArchivo(miinfo)
            print("la nueva compra se ha registrado exitosamente")
        elif x==2:
            miinfo=abrirarchivo()
